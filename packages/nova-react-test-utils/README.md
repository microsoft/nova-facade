...
You can also see that `satisfies WithNovaEnvironment<Operation, TypeMap, typeof Component>` is used to strongly type parameters. The `Operation` is used to validate `referenceEntries` prop parameter and passing `typeof Component` verifies that not only the name of the prop matches but also the returned value. If no `query` is defined `Operation` can be set to the `UnknownOperation` type exported from the package and passing `typeof Component` is not needed. The operation types are generated by the GraphQL compiler (either [apollo-react-relay-duct-tape-compiler](https://github.com/microsoft/graphitation/tree/main/packages/apollo-react-relay-duct-tape-compiler) or [relay-compiler](https://github.com/facebook/relay/tree/main/packages/relay-compiler)). The `TypeMap` type gives you strongly typed mock resolvers and can be generated using [typemap-plugin](https://github.com/microsoft/graphitation/tree/main/packages/graphql-codegen-typescript-typemap-plugin) that can be added to graphql codegen config file.
